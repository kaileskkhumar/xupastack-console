...
import { Search, CheckCircle2, XCircle, Loader2, ArrowRight, Server, Zap } from "lucide-react";
...
const BlockTest = () => {
  ...
  const handleTest = useCallback(async () => {
    ...
    try {
      ...
      // no-cors always returns opaque — if it doesn't throw, it's reachable
      setState("reachable");
    } catch {
      setState("blocked");
    }
            ...
            <AnimatePresence mode="wait">
              {state === "invalid" && (
                <ResultBanner key="invalid" variant="warn">
                  Must be a valid Supabase URL (e.g. <code className="font-mono">https://xxxxx.supabase.co</code>)
                </ResultBanner>
              )}
...
const ResultBanner = ({ children, variant }: { children: React.ReactNode; variant: "ok" | "blocked" | "warn" }) => {
  ...
  return (
    ...
    >
      {variant === "warn" ? (
        <p className="text-xs text-yellow-500">{children}</p>
      ) : (
        children
      )}
    </motion.div>
